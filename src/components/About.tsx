...
export default function About() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // Reveal animations
      const reveals = sectionRef.current?.querySelectorAll(".reveal, .reveal-left, .reveal-right, .reveal-scale, .stagger-children");
      reveals?.forEach((el) => {
        gsap.fromTo(
          el,
          { opacity: 0, y: 40 },
      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-start mb-24">
          ...
          <div className="reveal-left">
            ...
            <div className="space-y-4 leading-relaxed" style={{ color: "var(--text-secondary)" }}>
              <p>
                ...
                <span className="font-medium" style={{ color: "var(--text-primary)" }}>4 years of experience</span>{" "}
                building modern web applications, AI agents, and cloud-native systems from Dhaka, Bangladesh.
              </p>
              <p>
                Currently working on <span className="text-gradient font-semibold">Saturday AI</span> — a Jarvis-inspired autonomous AI agent running on a cloud VPS with 1,200+ skills, multi-model routing, and enterprise-grade tooling.
              </p>
              <p>
                I specialize in crafting robust full-stack applications, real-time systems, and scalable cloud architectures using React, Node.js, TypeScript, and Cloudflare's edge platform.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 stagger-children">
            {highlights.map((item) => (
              ...
              >
                <h3
                  className="font-[family-name:var(--font-display)] text-lg font-bold mb-2"
                  style={{ color: "var(--text-primary)" }}
                >
                  {item.title}
                </h3>
        ...
        <div id="skills">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <SkillColumn title="Languages" items={skills.languages} delay={0} />
            <SkillColumn title="Frameworks" items={skills.frameworks} delay={0.1} />
            <SkillColumn title="Tools & Cloud" items={skills.tools} delay={0.2} />
          </div>
        </div>
      </div>
    </section>
...
function SkillColumn({ title, items, delay: _delay }: { title: string; items: { name: string; level: number }[]; delay: number }) {
  return (
