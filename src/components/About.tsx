    ...
    <section id="about" className="py-16 md:py-24 lg:py-32">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 2xl:px-16" ref={ref}>
        <motion.div
          ...
          className="mb-16"
        >
          <p className="font-mono text-[0.625rem] uppercase tracking-[0.25em] text-accent mb-4">About Me</p>
          <h2 className="font-[var(--font-syne)] text-[clamp(3rem,6vw,7rem)] font-extrabold text-text-primary">Who I Am</h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          >
            ...
            <p className="text-[clamp(1rem,1.2vw,1.125rem)] leading-relaxed text-text-secondary">
          ...
          >
            {skills.map((group) => (
              <div key={group.category}>
                <h3 className="font-mono text-[0.625rem] uppercase tracking-[0.25em] text-accent mb-3">{group.category}</h3>
                <div className="flex flex-wrap gap-2">
                  {group.items.map((skill) => (
                    <span
                      key={skill}
                      className="bg-white/[0.05] backdrop-blur-sm border border-white/[0.08] rounded-full px-3 py-1 font-mono text-[0.625rem] uppercase tracking-[0.25em] text-text-secondary hover:text-accent hover:border-[rgba(229,9,20,0.3)] transition-all duration-300 cursor-default"
                    >
                      {skill}
                    </span>
                  ))}
